...
class MarinaRates:
    ...
    def calculate_base_price(self, length, days, vessel_type, beam=None):
        ...
        if effective_length > 35 and beam is not None:
            ...
            if days == 90:
                base_price = self.rates_table["3_MONTHS"].get(effective_length)
            elif days == 180:
                base_price = self.rates_table["6_MONTHS"].get(effective_length)
            elif days == 365:
                base_price = self.rates_table["ANNUAL"].get(effective_length)
            else:
                if days <= 90:
                    daily_rate = self.rates_table["DAILY"].get(effective_length)
                    base_price = daily_rate * days if daily_rate else None
                elif days <= 180:
                    daily_rate = self.rates_table["3_MONTHS"].get(effective_length) / 90
                    base_price = daily_rate * days if daily_rate else None
                elif days <= 365:
                    daily_rate = self.rates_table["6_MONTHS"].get(effective_length) / 180
                    base_price = daily_rate * days if daily_rate else None
                else:
                    daily_rate = self.rates_table["ANNUAL"].get(effective_length) / 365
                    base_price = daily_rate * days if daily_rate else None
...
rates_table_no_vat = {
    "DAILY": {7: 8.18, 8: 9.26, 9: 10.33, 10: 13.22, 11: 15.29, 12: 16.94, 13: 18.79, 14: 20.47,
              15: 26.05, 16: 28.10, 17: 30.39, 18: 32.23, 19: 33.50, 20: 36.36, 25: 48.76,
              30: 58.68, 35: 71.07},
    "3_MONTHS": {7: 681.82, 8: 739.67, 9: 805.75, 10: 929.75, 11: 1117.36, 12: 1301.65, 13: 1383.50,
                 14: 1512.40, 15: 1652.89, 16: 1776.69, 17: 1921.49, 18: 2066.12, 19: 2290.91,
                 20: 2602.48, 25: 3314.05, 30: 4545.45, 35: 5371.90},
    "6_MONTHS": {7: 1239.67, 8: 1392.56, 9: 1487.60, 10: 1776.86, 11: 2045.45, 12: 2396.69, 13: 2520.66,
                 14: 2735.54, 15: 3123.97, 16: 3264.46, 17: 3636.36, 18: 3933.88, 19: 4132.23,
                 20: 4934.71, 25: 6297.52, 30: 8437.19, 35: 9722.31},
    "ANNUAL": {7: 2272.73, 8: 2493.39, 9: 2561.98, 10: 3016.53, 11: 3574.38, 12: 4132.23, 13: 4561.98,
...
# Clean JSON in/out endpoint for n8n
@app.post("/estimate")
def estimate():
    ...
    if isinstance(quote, str):
        return jsonify(ok=False, error=quote), 200
    ...
    pricing = {
        ...
        "currency":        "EUR",
        "length_m": length, "beam_m": beam, "days": days,
...
def get_quote_with_vessel_type():
    try:
        ...
        params = (data.get('queryResult') or {}).get('parameters') or {}

        vessel_type = (params.get("vessel_type", "") or "monohull").lower()
        length = float(params.get("length", 0))
        beam = float(params.get("beam", 0))
        days = int(params.get("days", 0))
        stay_type = "short" if days < 183 else "long"

        quote = marina_rates.get_final_quote(length, beam, days, vessel_type, stay_type)
        if isinstance(quote, str):
            return jsonify({"fulfillmentText": quote}), 400

        response_text = (
            f"Here’s your quote:\n"
            f"Base Price (No VAT): €{quote['Base Price (No VAT)']}\n"
            f"T0 Tax: €{quote['T0']}\n"
            f"T5 Tax: €{quote['T5']}\n"
            f"Total Taxes: €{quote['Total Taxes']}\n"
            f"Total Price (With VAT): €{quote['Total (With VAT)']}"
        )
        return jsonify({"fulfillmentText": response_text})
    except Exception as e:
        return jsonify({"fulfillmentText": f"Internal Server Error: {e}"}), 500
